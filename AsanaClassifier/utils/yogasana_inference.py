...
from utils.cf_matrix import make_confusion_matrix, tqdm
import pickle
...
class YogaInference():
    ...
    def validate_batch(self, test_path,model="fine_tune"):
        ...
        pred_batch_probs = []
        actual_batch_vals = []
        for xb, label in tqdm(test_dl, title='Inferencing on images in given path..'):
        #for xb, label in test_dl:
            if model == "fine_tune":
...
def analyse_preds(act, pred, class_names, error_only=True):    
...
# df = analyse_preds(act, pred, error_only=True)
